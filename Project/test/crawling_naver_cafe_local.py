...
def save_data(data, fp):
    ...
    with open(fp, mode, newline='', encoding='utf-8-sig') as csvfile:  
...
def main():   
    timestamp_datetime_start = datetime.strptime('1000.08.08. 00:00', "%Y.%m.%d. %H:%M")
    timestamp_datetime_end = datetime.strptime('3000.08.08. 23:59', "%Y.%m.%d. %H:%M")
    ...
    for page_num in range(1, 25):
        filname = f'naver-cafe-{search_keyword}-{page_num}-'
        ...
        for title, link in zip(titles, links):
            ...
            print(f"page_num: {page_num}, title: {title}")
        save_data(posts_parsed, filname+'posts.csv')
        save_data(comments_parsed, filname+'comments.csv')
